...
// Icon Components
const PredictiveAnalyticsIcon = ({ selected }: { selected: boolean }) => (
  <span className={`mr-2 sm:mr-3 w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 inline-block align-middle ${selected ? "text-white" : "text-neutral-04"}`}>
...
const DataDashboardsIcon = ({ selected }: { selected: boolean }) => (
  <span className={`mr-2 sm:mr-3 w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 inline-block align-middle ${selected ? "text-white" : "text-neutral-04"}`}>
...
const GeospatialIntelligenceIcon = ({ selected }: { selected: boolean }) => (
  <span className={`mr-2 sm:mr-3 w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 inline-block align-middle ${selected ? "text-white" : "text-neutral-04"}`}>
...
const CrisisCommunicationIcon = ({ selected }: { selected: boolean }) => (
  <span className={`mr-2 sm:mr-3 w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 inline-block align-middle ${selected ? "text-white" : "text-neutral-04"}`}>
                      ...
                      <div className="flex items-center">
                        {(() => {
                          if (cat.name === "Predictive Analytics") {
                            return <PredictiveAnalyticsIcon selected={idx === selectedIndex} />;
                          } else if (cat.name === "Data Dashboards & Monitoring") {
                            return <DataDashboardsIcon selected={idx === selectedIndex} />;
                          } else if (cat.name === "Geospatial Intelligence") {
                            return <GeospatialIntelligenceIcon selected={idx === selectedIndex} />;
                          } else if (cat.name === "Crisis Communication Tools") {
                            return <CrisisCommunicationIcon selected={idx === selectedIndex} />;
                          } else {
                            return null;
                          }
                        })()}
