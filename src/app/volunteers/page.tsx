...
const assignments = Array(8).fill({ title: "Data Visualization Specialist ", desc: "Help design interactive dashboards for humanitarian data.", meta: "METADATA", href: "" });
    ...
    <>
      ...
      <section className="flex-1">
        ...
        <div className="mx-6 p-2 sm:p-16 m-6 bg-neutral-01 rounded-[20px]">
          ...
          <p className='mx-auto w-full sm:w-1/3 text-center my-4'>From data science to digital storytelling, we welcome volunteers with diverse tech and non-tech backgrounds.</p>
          <ul className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 place-items-center place-content-center">
            {skills.map(e => (
              <li key={e.name} className="rounded-lg p-4 w-full sm:m-3 contain-content bg-background">
        ...
        <div className="container grid items-center mx-auto p-4 m-6">
          ...
          <Carousel pages={1 + Math.floor((assignments.length - 1) / 6)} size={6} className="grid gap-4 grid-cols-1 md:grid-cols-3 place-items-center place-content-center">
            {assignments.map((e, i) => (
              <CI key={e.title} className="rounded-2xl bg-color-02 text-neutral-02 w-full p-8 contain-content flex flex-col gap-y-10 border" index={i}>
                <div>
                  <h5 className='text-white text-2xl font-bold'>{e.title}{i}</h5>
                  <p>{e.desc}</p>
                  <p>{e.meta}</p>
                </div>
                <Button className="mt-4 flex-none w-fit " variant='white' href={e.href}>Apply<span className="hidden md:inline">&nbsp;Now</span></Button>
              </CI>
            ))}
          </Carousel>
        </div>
